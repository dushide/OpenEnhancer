...
from models_save import DBONet
...
def main(args, device):
    ...
    def train(model, train_loader, valid_loader, device):
        ...
        try:
            ...
            test_X,test_Zv,test_Z, softmax_scores, test_truth_label = test(model, test_loader, device)
            ...
            if args.saveResp:
                ...
                x={f'test_X{i+1}': test_X[i] for i in range(n_view)}
                savemat={'test_indices': test_indices,
                                  'test_y': test_truth_label_open, 'test_Z': test_Z, 'test_Zv': test_Zv,
    ...
    def test(model, loader, device):
        ...
        with torch.no_grad():
            ...
            for batch in loader:
                ...
                for i in range(n_view):
                    ...
                    test_Zv[i].append(Zv[i].cpu().numpy())
        ...
        for i in range(n_view):
            ...
            test_Zv[i] = np.concatenate(test_Zv[i])
        return test_X,test_Zv,test_Z, prob, label
...
if __name__ == '__main__':
    ...
    lambda1s = lambda2s = [0.001, 0.005, 0.01, 0.05, 0.1, 0.5, 1, 5, 10]
