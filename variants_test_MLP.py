...
from variants import variant_MLP
...
def main(args, device):
    ...
    def train(model, train_loader, valid_loader, device):
        ...
        try:
            ...
            with open(file, "a") as f:
                f.write('batch_size:{},epoch:{},openness:{}, unseen_num:{},lambda1:{},lambda2:{}\n'.format(
                    args.batch_size, epoch, args.openness, unseen_num,  args.lambda1, args.lambda2))
    ...
    model = variant_MLP(n_feats, n_view, num_classes, thre, device)
...
if __name__ == '__main__':
    ...
    alpha = 0.1
    openness = 0.1
    file = './result/res_MLP_openness%.1f.txt' % (openness)
    hidden_dim=32
    path = './result/MLP_openness%.1f' % (openness)
    args.num_epoch = 100
    args.path = path
    args.file = file
    ...
    for ii in select_dataset:
            ...
            # features, labels= load_data(dataset_dict[ii], '/data/fangzh/code/data/')
            n_view = len(features)
